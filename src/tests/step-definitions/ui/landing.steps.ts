...
Then('showcase iframe features should be described', async function (this: CustomWorld) {
  ...
  const getIframe = await this.page.frameLocator('iframe');
  const getIframeContent = await getIframe.locator('div.poster-title').innerText();
  expect(getIframeContent).toContain('MOVIE SHOWCASE')
  logger.info('Showcase iframe features are described in the page: ' +  getIframeContent);
});
...
Then(
  'search placeholder should contain {string}',
  async function (this: CustomWorld, expectedText: string) {
...
Then('search should filter movie results', async function (this: CustomWorld) {
  ...
  const movieTitles = await results
    .filter({ has: this.page.locator('div h2') })
    .first()
    .allTextContents();
...
Then('prices should be in valid currency format', async function (this: CustomWorld) {
  const hasCurrency = await this.page.locator('p.MuiTypography-body2').filter({hasText: "Price: "}).all()
  console.log('hasCurrency length: ' + hasCurrency.length);
  for(const currency of hasCurrency){
    expect(await currency).toContainText('₹');
    expect(await currency.innerText()).toContain('₹');
    console.log(`Prices are displayed in currency format (₹): ` + await currency.innerText());
  }
...
Then('duration information should be visible for all movies', async function (this: CustomWorld) {
  const hasDuration = await this.page.locator('p.MuiTypography-body2').filter({hasText: "Duration: "}).all()
  //console.log('hasDuration length: ' + hasDuration.length);
  for(const duration of hasDuration){
    expect(duration).toBeTruthy();
  }
  logger.info('Duration information is visible for all movies ' + hasDuration.length);
});
...
Then('movie cards should be responsive on mobile', async function (this: CustomWorld) {
  //'div.MuiCard-root'
  const getMovieCard = await this.page.locator('div.MuiCard-root').first()
  await expect(getMovieCard).toBeVisible();

  const cardBox = await getMovieCard.boundingBox();
  const viewportSize = await this.page.viewportSize();
  console.log(' get a movie card width ' + cardBox?.width + ' <= get a page viewport size ' +  viewportSize!.width);
