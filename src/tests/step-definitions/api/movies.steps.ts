...
Then('response status should be {int}', async function (this: CustomWorld, expectedStatus: number) {
  if (expectedStatus >= 400) {
    ...
    logger.info(`Expected error status: ${expectedStatus}, got error: ${getErrorMessage(lastError)}`);
    return;
  }
  
  if (lastError) {
    throw new Error(`Expected status ${expectedStatus}, but got error: ${getErrorMessage(lastError)}`);
  }
...
When('user calls POST endpoint {string} with movie data:', async function (this: CustomWorld, _endpoint: string, dataTable) {
  moviesService = new MoviesService();
  lastError = null;
  ...
  try {
    const createRequest = {
      ...
      runtime: movieData.runtime && movieData.runtime.trim() ? parseInt(movieData.runtime) : undefined,
...
When('user calls PUT endpoint {string} with movie data:', async function (this: CustomWorld, endpoint: string, dataTable) {
  moviesService = new MoviesService();
  lastError = null;
...
When('user calls PUT endpoint with the created movie ID and movie data:', async function (this: CustomWorld, dataTable) {
  moviesService = new MoviesService();
  lastError = null;
  const movieId = this.getTestData('createdMovieId');
  const rows = dataTable.hashes();
